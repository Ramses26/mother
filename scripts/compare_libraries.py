...
def generate_reports(all_results: List[ComparisonResult], misplaced_files: List[MediaFile], output_dir: Path):
    ...
    with open(script_file, 'w', encoding='utf-8') as f:
        ...
        f.write('        (\n')
        f.write('            _cmd_exit=0\n')
        f.write('            "$@" 2>&1 || _cmd_exit=$?\n')
        f.write('            if [ $_cmd_exit -eq 0 ]; then\n')
        f.write('                mark_completed "$hash"\n')
        f.write('                log "${GREEN}OK${NC}: $desc"\n')
        f.write('            elif [ ! -e "${@: -2:1}" ]; then\n')
        f.write('                mark_completed "$hash"\n')
        f.write('                log "${YELLOW}SKIP${NC} [source gone - stale entry] $desc"\n')
        f.write('            else\n')
        f.write('                log_error "Failed (exit $_cmd_exit): $desc"\n')
        f.write('            fi\n')
        f.write('        ) &\n')
        f.write('    else\n')
        f.write('        local cmd_exit=0\n')
        f.write('        "$@" || cmd_exit=$?\n')
        f.write('        if [ $cmd_exit -eq 0 ]; then\n')
        f.write('            mark_completed "$hash"\n')
        f.write('            log "${GREEN}OK${NC}: $desc"\n')
        f.write('            ((COMPLETED++))\n')
        f.write('        elif [ ! -e "${@: -2:1}" ]; then\n')
        ...
        f.write('            ((SKIPPED++))\n')
        f.write('        else\n')
        f.write('            local cmd_exit_code=$?\n')
        f.write('            log_error "Failed (exit $cmd_exit): $desc"\n')
