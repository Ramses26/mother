#!/bin/bash
###############################################################################
# Movie Library Analysis Workflow - Project Mother
#
# Purpose: Generate inventories and run comparisons for movie libraries
#
# Usage:
#   ./analyze_movies.sh                    # Run full analysis
#   ./analyze_movies.sh --inventory-only   # Just generate inventories
#   ./analyze_movies.sh --compare-only     # Just run comparison (uses existing inventories)
#   ./analyze_movies.sh --4k               # Analyze 4K movies only
#   ./analyze_movies.sh --1080p            # Analyze 1080p movies only
#
...
main() {
    ...
    log_section "PROJECT MOTHER - MOVIE LIBRARY ANALYSIS"
    ...
    if [ "$do_compare" = true ]; then
        ...
        if [ "$do_1080p" = true ]; then
            local ali_1080p="$INVENTORY_DIR/ali_movies_1080p.json"
            local chris_1080p="$INVENTORY_DIR/chris_movies_1080p.json"

            if [ -f "$ali_1080p" ] && [ -f "$chris_1080p" ]; then
                log_info "Comparing 1080p movies..."
                local output_dir="$REPORTS_DIR/movies_1080p_$timestamp"
                mkdir -p "$output_dir"
        ...
        if [ "$do_4k" = true ]; then
            local ali_4k="$INVENTORY_DIR/ali_movies_4k.json"
            local chris_4k="$INVENTORY_DIR/chris_movies_4k.json"

            if [ -f "$ali_4k" ] && [ -f "$chris_4k" ]; then
                log_info "Comparing 4K movies..."
                local output_dir="$REPORTS_DIR/movies_4k_$timestamp"
                mkdir -p "$output_dir"
