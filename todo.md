...
- [ ] Apply the same two changes to **Terminus's** Alloy: `syslog_format = "rfc3164"` (if not already done there) and `host = "unifi-gomaa"` on its listener labels. Mother's is now `unifi-stuttler`; `service = "unifi"` is deliberately identical on both so a federated `{service="unifi"}` query returns both sites with `host` distinguishing them. Terminus's `alloy/config.alloy` already had uncommitted changes as of 2026-08-29 (noted above) — worth committing there at the same time.
