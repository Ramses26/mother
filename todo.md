...
## Nostromo / Plex
- [ ] **Nostromo Plex Kuma monitor exists but is SILENT** — monitor id 2 "Plex Monitor" (`http://10.0.0.250:32400/identity`, 60s, 3 retries) is live and green, but has **no notification attached** (`monitor_notification` has only hawser; `notification_sent_history` is empty — Kuma has never sent anything). Fix in the UI: Plex Monitor → Edit → tick "Server Notifications" → Save. Also consider setting that channel `is_default` so new monitors don't inherit the same silence. Two caveats: the channel targets Telegram chat `-5261916176`, **not** the `-5498616604` "Mother Notifications" infra group in CLAUDE.md; and Plex is only watched from Mother's Kuma, so if Mother is down nothing watches Plex.
...
- [x] Nostromo Plex systemd `MemoryMax=12G`/`MemoryHigh=10G` — legitimate, ported from Hathor, still in place.
- [x] **`WatchdogSec=300` was a mistake and caused a 2.5-day Plex crash-loop — removed 2026-09-01. DO NOT RE-ADD.** `WatchdogSec` is not a health check: it is an opt-in contract requiring the daemon to ping `sd_notify(WATCHDOG=1)`. Plex is not linked against libsystemd and has zero notify strings, and the unit is `Type=simple` (no notify socket), so the ping can never arrive — it degenerates into an unconditional SIGABRT every 300s. The claim that it "matches Hathor's pattern" was **false**: Hathor's override (Feb 10 2026) has no `WatchdogSec`; every other line matched, that one was invented. The note "no restart needed to apply" was also **backwards** — the directive is inert until the unit restarts, which is why it looked clean for ~11h until a root cron `service plexmediaserver start` at 2026-08-30 06:31 armed it. Result: killed every 5 min, 76 restarts, Aug 30 06:36 → Sep 1 13:07. Plex itself was never unhealthy; the "PLEX MEDIA SERVER CRASHED" lines were its signal handler reacting to systemd's SIGABRT.
- [x] Hand-written `StartLimitBurst`/`StartLimitIntervalSec` in Nostromo's override — **reverted 2026-09-01, don't re-add.** They were in `[Service]`; those keys belong in `[Unit]` (`systemd.unit(5)`), so `StartLimitIntervalSec=600` was ignored outright (journal: `Unknown key name ... in section 'Service'`) while legacy `StartLimitBurst=5` *was* honored and silently **loosened** Plex's own vendor policy. `/lib/systemd/system/plexmediaserver.service` already ships `StartLimitInterval=60s` + `StartLimitBurst=3` — the vendor covers this, no override needed. Note a start limiter would not have caught this incident anyway: with burst 5 it only trips when the restart period is under interval/4, and the loop's period was 311s. **Slow crash loops are only catchable by an external probe, which is why the Kuma notification above matters.**
