...
- [ ] **Gomaa UCG (192.168.1.1) syslog still not confirmed arriving.** Post-fix, every line landing on Mother is from the Stuttler site (`StuttlerNet ...`, plus AP `mcad[...]`) — ~4 lines/min, nothing identifiable from Gomaa. Could be genuinely not sending, or blocked over the VPN. Can't confirm from Mother without root (`tcpdump` on UDP 51514 needs sudo — permission denied as `alig`). **Next step:** run `sudo tcpdump -nn -i any udp port 51514` on Mother for a minute and check whether any packets arrive with source `192.168.1.1`. Also note the two sites are currently indistinguishable in Loki — per-site labeling by source IP is still deferred (see the comment in `configs/alloy/config.alloy`), so worth adding once delivery is confirmed.
