...
1. **Batch Sync** — Initial 160TB bulk transfer via `screen` sessions running rsync with `PARALLEL=16`. Scripts are generated by `compare_libraries.py` and stored in `reports/sync_actions_*.sh` and `reports/tv_sync_actions_*.sh`. **Movie sync is complete and disabled** (`/opt/mother/DISABLE_MOVIE_SYNC` sentinel prevents auto-restart). TV sync still running (Ali→Chris backfill, ~64k items).
...
3. **Nightly Reconciliation** — Three jobs run nightly to keep libraries in sync without risking data loss:
   - **TV gap scan (03:00 UTC)**: finds episodes on Synology missing from Unraid, queues them
   - **Movie gap scan (03:30 UTC)**: finds movie folders on Synology missing from Unraid, queues them
   - **Unraid dedup (04:45 UTC)**: calls Unraid Agent locally to find and delete lower-quality duplicates
...
- APScheduler tasks: daily summary, auto-retry (15min), history scanner (30min), **stall watchdog (15min)**, **TV gap scanner (03:00 UTC)**, **movie gap scanner (03:30 UTC)**, **Unraid dedup (04:45 UTC)**
...
**Unraid Dedup** (`nightly_unraid_dedup`): Nightly at 04:45 UTC. Calls `GET /scan?refresh=true` on
Unraid Agent (runs locally on Unraid, no CIFS overhead). For each duplicate group, deletes lower-quality
versions via `POST /delete` on Agent. Only deletes files marked `safe_to_delete=True` by Agent (multi-episode
files covering unique content are skipped). Sends Telegram summary when duplicates are removed.
This replaces the old per-webhook sweep logic that was causing data loss.
...
| 04:45 | sync-webhook | **Unraid dedup** (Agent /scan → /delete lower-quality duplicates; runs locally on Unraid) |
...
- Movie batch sync is **disabled** via `/opt/mother/DISABLE_MOVIE_SYNC` sentinel file. Deleting this file re-enables the `check-sync-health.sh` cron to restart the movie loop. TV sync continues at `PARALLEL=16`.
