...
3. **Upgraderr** — Custom quality upgrade automation service (replaces Huntarr). Active once initial sync reaches ~99%+. See `services/upgraderr/`. UI at port 9706.
...
- APScheduler tasks: daily summary, auto-retry (15min), history scanner (30min), **stall watchdog (15min)**, **gap scanner (nightly 03:30 UTC)**
...
**Gap Scanner** (`scan_library_gaps`): Nightly reconciliation at 03:30 UTC. Compares Synology HD-movies
NFS listing vs Unraid folder list fetched via Unraid Agent API. Queues any missing folders as `GapSync`
jobs. HD movies only — 4K excluded (TV excluded, TV batch sync is still active). Sends Telegram alert
listing all queued titles.
...
- Skips items with a `success` record in the last 7 days (avoids re-queuing stale-CIFS false positives)
- `UNRAID_AGENT_URL` and `UNRAID_AGENT_API_KEY` must be in sync-webhook's docker-compose env block
...
| 03:30 | sync-webhook | **Gap scanner** (Synology vs Unraid folder diff; queues missing HD movies) |
